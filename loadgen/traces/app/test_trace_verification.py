...
try:
    ...
    if tempo_resp.status_code == 200:
        ...
        tempo_delta = tempo_after - tempo_before
        print(f"Tempo: {tempo_delta:,.0f} spans (before: {tempo_before:,.0f}, after: {tempo_after:,.0f})")
except:
    pass
...
try:
    ...
    if vtraces_resp.status_code == 200:
        ...
        vtraces_delta = vtraces_after - vtraces_before
        print(f"VictoriaTraces: {vtraces_delta:,.0f} spans (before: {vtraces_before:,.0f}, after: {vtraces_after:,.0f})")
except:
    pass

print(f"\nExpected: 30 traces (1 span each) = 30 spans")
print(f"Tempo received: {tempo_after - tempo_before:.0f} spans")
print(f"VictoriaTraces received: {vtraces_after - vtraces_before:.0f} spans")
